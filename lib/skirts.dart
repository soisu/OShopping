...
class Skirts extends StatefulWidget {
  @override
  _SkirtPageState createState() => _SkirtPageState();
}

class _SkirtPageState extends State<Skirts> {
  final List<SkirtItem> services = [];
  String searchText = '';
  ...
  Future<void> _loadServices() async {
    final skirtData = await DatabaseHelper.instance.getSkirt();
    setState(() {
      services.clear();
      skirtData.forEach((brand_skirt) {
         services.add(SkirtItem(
          id: brand_skirt['id'],
          name: brand_skirt['name_skirt'],
          service: brand_skirt['brand_skirt'],
          imagePath: brand_skirt['imagePath_skirt'], 
          price: brand_skirt['price_skirt'],
          description: brand_skirt['description_skirt'],
        ));
      });
    });
  }
  ...
  Future<void> _addServiceToDatabase(String name, String brand, double price, String description, String imagePath) async {
    Map<String, dynamic> skirtData = {
      'name_skirt': name,
      'brand_skirt': brand,
      'price_skirt': price,
      'description_skirt': description,
      'imagePath_skirt': imagePath,
    };
    await DatabaseHelper.instance.insertSkirt(skirtData);
    _loadServices();
  }

  Future<void> _deleteSkirtFromDatabase(int id) async {
    await DatabaseHelper.instance.deleteSkirt(id);
    _loadServices();
  }
                ...
                return Container(
                  ...
                  child: ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () async {
                        await _deleteSkirtFromDatabase(item.id);
                        setState(() {
